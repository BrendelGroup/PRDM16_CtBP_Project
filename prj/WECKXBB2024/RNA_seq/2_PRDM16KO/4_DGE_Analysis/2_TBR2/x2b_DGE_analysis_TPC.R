...
counts_data <- read.table(file = './v1a_PRDM16_TPC_raw_counts.tsv', row.names = 1, sep = '\t', header = TRUE)

#Assigning a variable for the samples to conditions matrix
cat("Reading the samples to conditions table.\n\n\n")
colData <- read.table(file = './v1b_PRDM16_TPC_samples_to_conditions.tsv', row.names = 1, sep = '\t', header = TRUE)
...
write.table(res,file="./v2a_PRDM16_TPC_DGE_Matrix.tsv")
...
write.table(res2,file="./v2b_PRDM16_TPC_DGE_Matrix_NA_omitted.tsv")
...
write.table(res4,file="./v2c_PRDM16_TPC_DGE_Matrix_NA_omitted_filtered.tsv")
write.table(res5,file="./v2d_PRDM16_TPC_DGE_Matrix_significantly_upReg.tsv")
write.table(res6,file="./v2e_PRDM16_TPC_DGE_Matrix_significantly_downReg.tsv")
